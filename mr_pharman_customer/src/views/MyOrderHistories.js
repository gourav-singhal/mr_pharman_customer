...
const MyOrdersHistories = (props) => {
  ...
  const renderItem = ({ item }) => (
    <View style={{ marginBottom:20, margin:5 }} >
      <TouchableOpacity activeOpacity={1} onPress={order_details.bind(this, item)}>
        <CardView
          cardElevation={5}
          cardMaxElevation={5}
          cornerRadius={10}>
          { item.status_id <= 6 ?
            ...
            :
            <View style={{ alignItems:'center', width:'100%'}}>
              <View style={{ width:'100%', alignItems:'center', backgroundColor:colors.error , borderColor:colors.theme_fg, borderWidth:1, padding:10 , flexDirection:'row'}} >
                <View style={{ alignItems:'flex-start', justifyContent:'center', width:'70%'}}>
                  <Text style={{ fontSize:14, color:colors.theme_fg_three, fontFamily:bold  }}>{ item.status }</Text>
                </View>
              </View>
  ...
  <SafeAreaView style={styles.container}>
    ...
    {order_list.length >0 ?
      ...
      <View style={{ justifyContent: 'center', alignItems:'center' }}>
        <Text style={{ fontSize:14, color:colors.theme_fg_two, fontFamily:bold }}>No Orders created yet...</Text>
      </View>
    }
