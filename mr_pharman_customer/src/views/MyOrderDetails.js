...
import {  bold, regular, medicine, customer_order_details, api_url, status_change } from '../config/Constants';
...
const MyOrderDetails = () =>{
	...
	const change_status = async(slug) => {
		...
		await axios({
			...
			data:{ order_id:data.id, slug:slug }
		})
		.then(async response => {
			setLoading(false);
				...
				<ScrollView>
					...
					<CardView
						...
						<View style={{ padding:20 }}>
							...
							<View style={{ margin:10 }} />
							{data.status_id < 5 &&
							<TouchableOpacity onPress={change_status.bind(this,'cancelled_by_customer')} style={{ padding:5, width:'96%', marginLeft:'2%', marginRight:'2%', backgroundColor:colors.error, borderRadius:5, alignItems:'center', justifyContent:'center' }}>
