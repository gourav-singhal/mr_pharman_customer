          ...
          <TouchableOpacity activeOpacity={1} onPress={change_category.bind(this,data.id,data.category_name)}>
            <CardView
              ...
              <View  style={{  width:100, alignItems:'center', justifyContent:'center' }}>
                <Image source={{ uri : img_url + data.category_image}} style={{ height:50, width:60 }} />
                <View style={{ margin:5 }} />
                <Text numberOfLines={1} style={[ active_category == data.id ? styles.active_category_fg : styles.inactive_category_fg]}>{data.category_name}</Text>
              </View>
            </CardView>
          </TouchableOpacity>
        </View>
      ...
      return (
        <TouchableOpacity activeOpacity={1} onPress={view_products.bind(this,data.id,data.sub_category_name)} style={{ flexDirection:'row', marginBottom:10, padding:10, backgroundColor:light_colors[index], borderRadius:10, borderLeftWidth:5, borderColor:colors.theme_fg}}>
          <View style={{ width:'60%', padding:10}}>
            <Text style={{ fontSize:16, color:colors.theme_fg_two, fontFamily:bold}}>{data.sub_category_name}</Text>
            <View style={{ margin:10 }} />
            <Text style={{ fontSize:14, color:colors.theme_fg_two, fontFamily:regular, textDecorationLine: 'underline'}}>View Products</Text>
          </View>
